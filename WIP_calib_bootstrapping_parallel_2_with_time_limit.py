...
def run_calibration_over_conditions_for_bootstrapping_run_with_timeout(input_dict_for_run):
    run_id=list(input_dict_for_run.keys())[0]
    #print("Bootstrapping run: {}".format(run_id))
    time_limit=input_dict_for_run[run_id]["timeout"]
    #if run_id!="mean_noNoise":
     #   time_limit=None

    if time_limit is not None:
        print("Bootstrapping run: {} - TL: {}".format(run_id,time_limit))
        try:
            result=func_timeout(time_limit, run_calibration_over_conditions_for_bootstrapping_run, args=(input_dict_for_run,))
            return(result)
        except FunctionTimedOut:
            print("Timeout run: {}".format(run_id))
            return({run_id:None})
    else:
        print("Bootstrapping run: {}".format(run_id))
        result=run_calibration_over_conditions_for_bootstrapping_run(input_dict_for_run)
        return(result)
...
def run_calibration_over_conditions_for_bootstrapping_run(input_dict_for_run):
    ...
    #regressed_compartment_sizes_1=regression_on_compartment_sizes(Comp_sizes=compartment_sizes_from_calibration_1,conditions=input_dict["conditions"],growth_rates=growth_rates,monotonous_quadratic=False,plotting=False)
    #regressed_pg_fractions_1=regression_on_pg_fractions(PG_sizes=pg_fractions_from_calibration_1,conditions=input_dict["conditions"],growth_rates=growth_rates,monotonous_quadratic=False,plotting=False)
    
    conditions_for_compartment_regression=input_dict["conditions_for_compartment_regression"]
    if conditions_for_compartment_regression is not None:
        regressed_compartment_sizes_1=regression_on_compartment_sizes(Comp_sizes=compartment_sizes_from_calibration_1,conditions=conditions_for_compartment_regression,growth_rates=growth_rates,monotonous_quadratic=False,plotting=False)
        regressed_pg_fractions_1=regression_on_pg_fractions(PG_sizes=pg_fractions_from_calibration_1,conditions=conditions_for_compartment_regression,growth_rates=growth_rates,monotonous_quadratic=False,plotting=False)
    ...
    if input_dict["regression_on_compartments"]:
...
def main(conditions,number_samples,n_parallel_processes=None,chunk_size=1,import_initial_parameters=False,regression_on_compartments=True,conditions_for_compartment_regression=None,timeout=None):
    ###prep
    ...
    for runs_to_execute in list_of_runs_to_execute:
        ...
        if n_parallel_processes!=1:
            ...
            pool=Pool(n_jobs)
            bootstrapping_runs=pool.imap_unordered(run_calibration_over_conditions_for_bootstrapping_run_with_timeout,input_dicts)
            #bootstrapping_runs=pool.imap_unordered(run_calibration_over_conditions_for_bootstrapping_run,input_dicts)
        else:
            bootstrapping_runs=[run_calibration_over_conditions_for_bootstrapping_run_with_timeout(input_dict) for input_dict in input_dicts]
            ...
            for condition in conditions:
                for params in [Reconstructed_Proteomes,Corrected_Proteomes,Compartment_sizes,Pg_fractions,Specific_Kapps,Process_efficiencies,Default_Kapps]:
                    ...
                    if len(list(df.columns))>0:
                        df["mean_run_param"]=[numpy.nanmean([df.loc[i,j] for j in df.columns if j.startswith("run_")]) for i in df.index]
                if "mean_noNoise" in list(Specific_Kapps_directions[condition].columns):
                    Specific_Kapps_directions[condition]["mean_run_param"]=Specific_Kapps_directions[condition]["mean_noNoise"]
        for condition in conditions:
...
if __name__ == "__main__":
    ...
    main(n_parallel_processes=None,
         ...
         conditions_for_compartment_regression=None,
         )
