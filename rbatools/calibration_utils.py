...
def perform_simulations_old(condition,
    ...
    if "Prokaryotic" in sims_to_perform:
        ...
        if Exchanges_to_impose is not None:
            rba_session.Problem.set_lb({exrx: Exchanges_to_impose[exrx]["LB"] for exrx in list(Exchanges_to_impose.keys()) if not pandas.isna(Exchanges_to_impose[exrx]["LB"])})
            rba_session.Problem.set_ub({exrx: Exchanges_to_impose[exrx]["UB"] for exrx in list(Exchanges_to_impose.keys()) if not pandas.isna(Exchanges_to_impose[exrx]["UB"])})
        mumax_prok = rba_session.find_max_growth_rate(precision=Mu_approx_precision,
                                                      max_value=max_mu_in_dichotomy,
...
def efficiency_correction_2(enzyme_efficiencies,
                               simulation_results,
                               protein_data,
                               rba_session,
    ...
    # 1. Sum all predicted protein iso-form concentrations to determine predicted proto-protein concentration
    proto_protein_predicted={}
    for protein in simulation_results["Proteins"].index:
        proto_ID=rba_session.get_protein_information(protein=protein)["ProtoID"]
        if proto_ID in proto_protein_predicted:
            proto_protein_predicted[proto_ID]+=simulation_results["Proteins"].loc[protein,condition_to_look_up]
        else:
            proto_protein_predicted[proto_ID]=simulation_results["Proteins"].loc[protein,condition_to_look_up]

    # 2. Determine protein-prediction error (RSS) over all proto-proteins
    squared_residuals=[]
    for proto_protein in proto_protein_predicted.keys():
        if proto_protein in protein_data.index:
    ...
    for enzyme in rba_session.get_enzymes():
        ...
        iso_enzymes=[enzyme]+rba_session.get_enzyme_information(enzyme=enzyme)["Isozymes"]
        #iso_enzymes=[enzyme]
        for iso_enzyme in iso_enzymes:
            if only_consider_misprediction_for_predicted_nonzero_enzymes:
                if iso_enzyme not in simulation_results["Enzymes"].index:
                    continue
                elif numpy.isfinite(simulation_results["Enzymes"].loc[iso_enzyme,condition_to_look_up])==False:
                    continue
            ...
            for subunit in rba_session.get_enzyme_information(enzyme=iso_enzyme)["Subunits"].keys():
                ...
                if (subunit in simulation_results["Proteins"].index) and (proto_ID in protein_data.index):
                    ...
                    predicted_concentration=proto_protein_predicted[proto_ID]
                    if (numpy.isfinite(measured_concentration)) and (numpy.isfinite(predicted_concentration)) and (measured_concentration!=0) and (predicted_concentration!=0):
                        if proto_ID not in su_concentrations['Subunits']:
    ...
    process_machinery_regression_results={}
...
#            su_concentrations['Regression_coefficient']=regression_results['Regressor'].coef_[0][0]
    ...
    for enzyme in enzyme_regression_results.keys():
        ...
        if enzyme in enzyme_efficiencies["Enzyme_ID"]:
            ...
            if correct_default_kapp_enzymes:
                ...
                respective_proto_reaction=associated_reaction.split("_duplicate_")[0]
                if respective_proto_reaction in simulation_results["Reactions"].index:
                    flux=simulation_results["Reactions"].loc[respective_proto_reaction,condition_to_look_up]
                    try:
                        if flux < 0:
                            flux_direction=-1
    ...
    return({"Sum_of_squared_residuals":sum(squared_residuals),
            ...
            "Process_MispredictionFactors":{i:process_machinery_regression_results[i]['Regression_coefficient'] for i in process_machinery_regression_results.keys()},
...
def determine_calibration_flux_distribution_new(rba_session,
    ...
    if parsimonious_fba:
        ...
        if use_bm_flux_of_one:
            if BMfluxOld >= 1.0:
                ...
                if not rba_session.FBA.Solved:
                    ...
                    bm_flux_to_impose=BMfluxOld
                else:
                    print("{} - Parsi BM-flux: {}".format(condition,1.0))
                    bm_flux_to_impose=1.0
            else:
                rba_session.FBA.set_lb({BMfunction: BMfluxOld})
                rba_session.FBA.set_ub({BMfunction: BMfluxOld})
                rba_session.FBA.solve_lp()
        ...
        print("{} - Parsi Objective value 1: {}".format(condition,rba_session.FBA.ObjectiveValue))
        if len(list(protein_milp_input.keys()))!=0:
            old_parsi_objective=rba_session.FBA.get_objective()

            FluxSumMatrix=numpy.ones((1,len(list(old_parsi_objective.keys()))+1))
            FluxSumMatrix[0,-1]=-1.0
            FluxSum_col_names=list(list(old_parsi_objective.keys())+['FluxSum'])
            FluxSum_row_names=['FluxSum_constraint']

            LBorig=rba_session.FBA.get_lb()
            UBorig=rba_session.FBA.get_ub()
            matrix_to_add = ProblemMatrix()
            matrix_to_add.A = scipy.sparse.coo_matrix(FluxSumMatrix)
            matrix_to_add.b = numpy.array([0.0])
            matrix_to_add.f = numpy.array([0.0]*len(FluxSum_col_names))
            matrix_to_add.LB = numpy.array([LBorig[i] for i in list(old_parsi_objective.keys())]+[0.0])
            matrix_to_add.UB = numpy.array([UBorig[i] for i in list(old_parsi_objective.keys())]+[10000000000000.0])
            matrix_to_add.row_signs = ['E']
            matrix_to_add.row_names = FluxSum_row_names
            matrix_to_add.col_names = FluxSum_col_names
            matrix_to_add.var_types = ['C']*len(FluxSum_col_names)

            matrix_to_add.map_indices()
            rba_session.FBA.LP.add_matrix(matrix=matrix_to_add)

            rba_session.FBA.clear_objective()
            rba_session.FBA.set_objective({'FluxSum':1.0})
            rba_session.FBA.set_lb(rxn_LBs)
            rba_session.FBA.set_ub(rxn_UBs)
            rba_session.FBA.set_lb({BMfunction: bm_flux_to_impose})
            rba_session.FBA.set_ub({BMfunction: bm_flux_to_impose})
            if rba_session.FBA.Solved:
                print("{} - Parsi Objective value 1.2: {}".format(condition,rba_session.FBA.ObjectiveValue))
            else:
                print("FSerror")
            ### protein stuff here ###
            if len(list(protein_milp_input.keys()))!=0:
    ...
    #                for j in list(fw_milp_score_ids+bw_milp_score_ids):
                ...
                assembled_matrix_objective=list([0.0]*len(list(fw_milp_rxn_ids+bw_milp_rxn_ids+gene_activity_matrix_col_ids))+[0.0])
    ...
    #
            ...
            rba_session.FBA.solve_lp()
            if rba_session.FBA.Solved:
                print("{} - Parsi Objective value 2: {}".format(condition,rba_session.FBA.ObjectiveValue))
            else:
                print("MILP dysfunctional - 1")
            original_flux_sum=rba_session.FBA.ObjectiveValue

            rba_session.FBA.clear_objective()
            rba_session.FBA.set_objective({'Global_activity_score':-1})
            rba_session.FBA.set_lb(rxn_LBs)
            rba_session.FBA.set_ub(rxn_UBs)
            rba_session.FBA.set_lb({BMfunction: bm_flux_to_impose})
            rba_session.FBA.set_ub({BMfunction: bm_flux_to_impose})
            rba_session.FBA.solve_lp()
            activity_score = rba_session.FBA.SolutionValues['Global_activity_score']

            if rba_session.FBA.Solved:
                print("{} - Activity score - 1: {} / {}".format(condition,activity_score,rba_session.FBA.get_ub('Global_activity_score')['Global_activity_score']))
            else:
                print("MILP dysfunctional - 2")
            ...
            rba_session.FBA.set_lb({BMfunction: bm_flux_to_impose})
            rba_session.FBA.set_ub({BMfunction: bm_flux_to_impose})
            rba_session.FBA.set_ub({'FluxSum': original_flux_sum})
            rba_session.FBA.solve_lp()
            activity_score = rba_session.FBA.SolutionValues['Global_activity_score']

            if rba_session.FBA.Solved:
                print("{} - Activity score - 2: {} / {}".format(condition,activity_score,rba_session.FBA.get_ub('Global_activity_score')['Global_activity_score']))
            else:
                print("MILP dysfunctional - 2.2")

            #rba_session.FBA.clear_objective()
            #rba_session.FBA.set_objective({'FluxSum':1.0})
            #rba_session.FBA.set_lb({'Global_activity_score':activity_score})
            #rba_session.FBA.set_ub({'Global_activity_score':activity_score})
            #rba_session.FBA.set_lb(rxn_LBs)
            #rba_session.FBA.set_ub(rxn_UBs)
            #rba_session.FBA.set_lb({BMfunction: bm_flux_to_impose})
            #rba_session.FBA.set_ub({BMfunction: bm_flux_to_impose})
...
def determine_calibration_flux_distribution_2(rba_session,
                                            ...
                                            biomass_function,
                                            target_biomass_function,
                                            #rRNA_target_rna_weight_ratio,
                                            parsimonious_fba,
                                            rxns_to_ignore_when_parsimonious
                                            ):
    if target_biomass_function:
        ### modify model with targets
...
def estimate_specific_enzyme_efficiencies(rba_session, 
                                          ...
                                          #rRNA_target_rna_weight_ratio=None,
